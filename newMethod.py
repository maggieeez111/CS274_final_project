"""
ZeRA and ZeRA-DT implementation
"""
...
class ZeRARetriever:
    """Zero-shot Response-Aware retriever implementation."""
...
class ZeRADTRetriever:
    ...
    def search(self, query, context, turn_number, top_k=100):
        ...
        # 1. Get BM25 results directly
        bm25_results = self._get_bm25_results(query, top_k*2)

        # 2. Adjust weights based on turn number
    ...
    def _get_term_expanded_query(self, query, candidate_docs):
        """term expansion focused on effectiveness."""
        query_terms = set(re.findall(r'\w+', query.lower()))
...
def run_evaluation(data_dir, use_manual=False, models=None, verbose=False, cuda=False):
    ...
    if "ZeRA" in models:
        ...
        zera.index_corpus(evaluator.corpus)
    ...
    if "ZeRA-DT" in models:
        ...
        zera_dt.index_corpus(evaluator.corpus)
